...
async def toonworld4all(url: str):
    ...
    for no, tl in enumerate(titles):
        ...
        for tl, sl in zip(links, lstd[no]):
            prsd += f"<a href='{sl}'>{tl.string}</a>, " if not isinstance(sl, Exception) else str(sl)
        prsd = prsd[:-2]
    return prsd

async def tamilmv(url: str):
    cget = create_scraper().request
    resp = cget("GET", url)
...
# -------------------- API Endpoints --------------------
def endpoint(func, source_name):
    async def wrapper(url: str = Query(..., description=f"{source_name} URL")):
        try:
            result = await func(url)
            return JSONResponse({"success": True, "source": source_name, "data": result})
        except Exception as e:
            logger.exception(e)
            return JSONResponse({"success": False, "error": str(e)})
    return wrapper

router.get("/sharespark")(endpoint(sharespark, "ShareSpark"))
router.get("/skymovieshd")(endpoint(skymovieshd, "SkyMoviesHD"))
router.get("/cinevood")(endpoint(cinevood, "CineVood"))
router.get("/kayoanime")(endpoint(kayoanime, "KayoAnime"))
router.get("/toonworld4all")(endpoint(toonworld4all, "ToonWorld4All"))
router.get("/tamilmv")(endpoint(tamilmv, "TamilMV"))

# Include router in app
app.include_router(router, prefix="/scrap", tags=["Scrapers"])
